...
saving_path=['I' path(2:end)];
mkdir(saving_path);
cd(path);
%mkdir([saving_path '\SavedImages']);
while 1==1
    try
        ...
        for ii=3:length(A)
            if ( ~isempty(strfind(A(ii).name,'F_')) & ...
                    ...
                    (current_timestamp-str2num(A(ii).name(end-14:end))>2*10^3)) ...% Old line: (str2double(A(ii).name(end-8:end))<(mrFnumber-100)) ) ...
                    | ( ~isempty(strfind(A(ii).name,'R_')) & ...
                    isempty(strfind(A(ii).name,'R_r')) &...
                    (current_timestamp-str2num(A(ii).name(end-14:end))>2*10^3))% Old line: (str2double(A(ii).name(end-8:end))<(mrRnumber-100)))
            ...
            if (    ~isempty(strfind(A(ii).name,'F_r'))& ...
                    (current_timestamp-str2num(A(ii).name(end-14:end))>2*10^3)) ...% Old line: (str2double(A(ii).name(end-8:end))<(mrFnumber-100)) ) ...
                    | ( ~isempty(strfind(A(ii).name,'R_r')) &...
                    (current_timestamp-str2num(A(ii).name(end-14:end))>2*10^3))% Old line: (str2double(A(ii).name(end-8:end))<(mrRnumber-100)))
                   
                movefile([A(ii).name],[saving_path]);
                  display('Moved');
           end
